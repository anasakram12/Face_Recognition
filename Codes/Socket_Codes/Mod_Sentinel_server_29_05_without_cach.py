...
class FaceRecognitionServer:
    ...
    def recognize_face(self, image_data, camera_id=None):
        """Recognize face with optimized performance and improved input/output tracking"""
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        request_id = str(uuid.uuid4())[:8]  # Add a unique request ID for tracking
        ...
        try:
            ...
            # Prepare camera ID prefix with added request_id for uniqueness
            ...
            if len(faces) == 0:
                ...
                if unrecognized_img is None:
                    result = {
                        ...
                        "request_id": request_id  # Include request_id in the result
                    }
                    self.log_recognition_details(result, camera_id)
                    return result
                
                # Add request_id to output filename too for uniqueness
            ...
            if best_match and best_similarity > self.threshold:
                ...
                # Use a copy of matched image to avoid potential race conditions
                matched_image = cv2.imread(primary_image_path).copy()
                
                # Ensure output filename has the request_id for guaranteed uniqueness
            ...
            # Save to Out folder with camera ID prefix and request_id
            output_filename = self.save_image(unrecognized_img, "Get/Out", f"{prefix}unrecognized")
