...
from PyQt5.QtWidgets import (QLabel, QVBoxLayout, QHBoxLayout, QFileDialog, 
                           QMessageBox, QTableWidget, QTableWidgetItem, QMenuBar, 
...
SERVER_URL = "http://192.168.14.205:3005"  # Replace with your server's IP address
...
class FaceRecognitionClient(QtWidgets.QWidget):
    def __init__(self):
        super().__init__()
        self.config_file = 'client_config_61_62.ini'
            ...
            try:
                ...
                if response.status_code == 200:
                    QMessageBox.information(
                        ...
                        f"Threshold has been set to {new_threshold:.2f}"
                    )
            except requests.RequestException as e:
                QMessageBox.warning(self, "Error", f"Failed to set threshold: {str(e)}")
    ...
    def start_watching_folder(self):
        if self.watch_folder:
            ...
            self.observer.start()
            QMessageBox.information(
                self, "Folder Watching",
                f"Now watching folder: {self.watch_folder}"
            )
        ...
        try:
            ...
            if response.status_code == 200:
                QMessageBox.information(
                    ...
                    "Face database has been reloaded successfully."
                )
        except requests.RequestException as e:
            QMessageBox.warning(self, "Error", f"Failed to reload database: {str(e)}")
    ...
    def process_new_image(self, file_path):
        try:
            ...
            # Check for camera number and draw boxes
            camera_number = None
            if 'camera61' in file_path:
                camera_number = 61
                color = QtGui.QColor('blue')  # Use QColor here
            elif 'camera62' in file_path:
                camera_number = 62
                color = QtGui.QColor('purple')  # Use QColor here
    ...
    def refresh_table(self):
        try:
            ...
            QMessageBox.warning(
                self, "Error",
                f"Failed to refresh display: {str(e)}"
            )
...
def main():
    try:
        ...
        QMessageBox.critical(
            ...
            f"Could not connect to the server at {SERVER_URL}.\n"
