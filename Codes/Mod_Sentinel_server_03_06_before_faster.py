...
class FaceRecognitionServer:
    def __init__(self):
        ...
        # *** NEW: Recent recognition cache (5-minute sliding window) ***
        self.recent_recognition_cache = OrderedDict()  # Key: embedding_id, Value: (embedding, result, timestamp)
        self.recent_cache_duration = 300  # 5 minutes in seconds
        self.recent_cache_similarity_threshold = 0.70  # High threshold for recent cache matches
        self.cache_hit_count = 0  # Track cache hits for statistics
        self.cache_miss_count = 0  # Track cache misses for statistics
        ...
        # *** NEW: Start cleanup thread for recent cache ***
        recent_cache_cleanup_thread = threading.Thread(target=self.cleanup_recent_cache, daemon=True)
        recent_cache_cleanup_thread.start()

    def cleanup_recent_cache(self):
        """Periodically clean up expired entries from recent recognition cache"""
        while True:
            ...
            try:
                ...
                expired_keys = [
                    key for key, (embedding, result, timestamp) in self.recent_recognition_cache.items()
                    if current_time - timestamp > self.recent_cache_duration
                ]
                for key in expired_keys:
                    if key in self.recent_recognition_cache:
                        del self.recent_recognition_cache[key]
                ...
                if expired_keys:
                    print(f"[RECENT CACHE] Cleaned up {len(expired_keys)} expired entries")
                    
            except Exception as e:
                print(f"Error in recent cache cleanup: {e}")

    def add_to_recent_cache(self, query_embedding, result):
        """Add a successful recognition to the recent cache"""
        try:
            # Only cache successful matches 0.6
            if result.get('status') == 'matched' and result.get('confidence', 0) > 0.5:
                # Create a unique key for this embedding
                embedding_key = f"recent_{int(time.time() * 1000)}_{len(self.recent_recognition_cache)}"
                
                # Normalize the embedding for consistent comparison
                normalized_embedding = query_embedding / np.linalg.norm(query_embedding)
                
                # Store in cache
                self.recent_recognition_cache[embedding_key] = (
                    normalized_embedding,
                    result.copy(),  # Store a copy of the result
                    time.time()
                )
                
                # Limit cache size to prevent memory issues (max 100 recent entries)
                if len(self.recent_recognition_cache) > 100:
                    # Remove oldest entry
                    self.recent_recognition_cache.popitem(last=False)
                    
                print(f"[RECENT CACHE] Added entry for {result.get('matched_filename', 'unknown')} (cache size: {len(self.recent_recognition_cache)})")
                
        except Exception as e:
            print(f"Error adding to recent cache: {e}")

    def check_recent_cache(self, query_embedding):
        """Check if the query embedding matches any recent recognition"""
        try:
            if not self.recent_recognition_cache:
            ...
            # Check against all recent embeddings
            for embedding_key, (cached_embedding, cached_result, timestamp) in self.recent_recognition_cache.items():
                # Skip expired entries (they'll be cleaned up later)
                if current_time - timestamp > self.recent_cache_duration:
                    continue
                ...
                if similarity > self.recent_cache_similarity_threshold and similarity > best_similarity:
                    best_similarity = similarity
                    best_match = cached_result.copy()
                    best_match['recent_cache_hit'] = True
                    best_match['recent_cache_similarity'] = similarity
                    best_match['cache_age_seconds'] = current_time - timestamp
            
            if best_match:
                self.cache_hit_count += 1
                print(f"[RECENT CACHE HIT] Similarity: {best_similarity:.3f}, Age: {best_match['cache_age_seconds']:.1f}s")
            else:
                self.cache_miss_count += 1
            ...
            print(f"Error checking recent cache: {e}")
            self.cache_miss_count += 1
            return None

    def get_cache_statistics(self):
        """Get statistics about the recent recognition cache"""
        current_time = time.time()
        active_entries = 0
        expired_entries = 0
        
        for embedding_key, (embedding, result, timestamp) in self.recent_recognition_cache.items():
            if current_time - timestamp <= self.recent_cache_duration:
                active_entries += 1
            else:
                expired_entries += 1
        ...
        return {
            "total_cache_entries": len(self.recent_recognition_cache),
            "active_entries": active_entries,
            "expired_entries": expired_entries,
            "cache_duration_seconds": self.recent_cache_duration,
            "cache_similarity_threshold": self.recent_cache_similarity_threshold,
    ...
    def process_recognition_queue(self):
        """Worker thread to process face recognition requests with improved tracking"""
        while True:
            try:
                ...
                # Store result in cache with request_id as key
    ...
    def log_recognition_details(self, result, camera_id=None):
        ...
        try:
            ...
            ws_event_data = {
                ...
                "recent_cache_hit": result.get('recent_cache_hit', False),  # NEW: Add cache hit info
                "cache_age_seconds": result.get('cache_age_seconds', 0)  # NEW: Add cache age info
            }
    ...
    def _send_log_to_apis(self, result, camera_id):
        ...
        try:
            ...
            log_entry = {
                ...
                "recent_cache_hit": result.get('recent_cache_hit', False)  # NEW: Add cache info to logs
            }
    ...
    def save_image(self, img, folder, prefix):
        """Save image with optimized I/O and guaranteed uniqueness"""
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S_%f")  # Add milliseconds for uniqueness
        filename = f"{prefix}_{timestamp}.jpg"
        filepath = os.path.join(folder, filename)
        
        # Add lock for thread safety during file operations
        with threading.Lock():
            ...
            thread = threading.Thread(target=save_image_task, daemon=True)
            thread.start()
            thread.join(0.5)  # Wait briefly for the file to be saved
        
        return filename

    def save_unrecognized_face(self, img, face, input_filename, camera_id=None):
        """Save unrecognized face with optimized I/O and better uniqueness"""
        ...
        # Use threading to save image without blocking
        with threading.Lock():  # Add lock for thread safety
            def save_image_task():
                cv2.imwrite(filepath, img)
    ...
    def recognize_face(self, image_data, camera_id=None):
        """Recognize face with recent recognition cache optimization"""
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        ...
        try:
            ...
            if len(faces) == 0:
                # Handle no face detected (same as before)
            ...
            # *** NEW: Check recent recognition cache first ***
            recent_match = self.check_recent_cache(query_embedding)
            if recent_match:
                print(f"[CACHE HIT] Found recent match with similarity {recent_match.get('recent_cache_similarity', 0):.3f}")
                
                # Update the result with current request info
                recent_match.update({
                    "input_filename": input_filename,
                    "request_id": request_id,
                    "camera_id": camera_id,
                ...
                # Create output image from the cached matched image
                if recent_match.get('matched_filename'):
                    # Find the original matched image
                    matched_image_path = None
                    for identity, data in self.face_db.items():
                        if os.path.basename(data.get('image_path', '')) == recent_match['matched_filename']:
                            matched_image_path = data['image_path']
                            break
                    
                    if matched_image_path and os.path.exists(matched_image_path):
                        matched_image = cv2.imread(matched_image_path)
                        output_filename = self.save_image(matched_image, "Get/Out", f"{prefix}cache_match")
                        recent_match["output_filename"] = output_filename
                        
                        # Update matched image in base64
                        _, buffer = cv2.imencode('.jpg', matched_image)
                        recent_match["matched_image"] = base64.b64encode(buffer).decode('utf-8')
                
                # Log and return the cached result
                self.log_recognition_details(recent_match, camera_id)
                return recent_match

            # *** If no recent cache hit, proceed with full database search ***
            print("[CACHE MISS] Checking full database...")
            
            # Normalize query embedding for full database search
            normalized_query_embedding = query_embedding / np.linalg.norm(query_embedding)

            # Full database search (existing logic)
            best_match = None
            best_similarity = -1
            best_identity = None
            ...
            if best_match and best_similarity > self.threshold:
                # Save multi-image variant if similarity is above 0.5
                ...
                # Create output
                matched_image = cv2.imread(primary_image_path).copy()
                ...
                result = {
                    ...
                    "camera_id": camera_id,
                    "request_id": request_id,
                    "from_full_database": True  # Indicator that this came from full DB search
                }
                
                # *** NEW: Add this successful match to recent cache ***
                self.add_to_recent_cache(query_embedding, result)
                
                self.log_recognition_details(result, camera_id)
                return result
            
            # No match found (existing logic)
    ...
    def _process_batch_file(self, filename, image_id):
        """Process a single file from a batch with improved tracking"""
        # Generate a unique request ID for this batch file
        request_id = str(uuid.uuid4())[:8]
        ...
        try:
            ...
            # Create a copy of the image to prevent race conditions
            img = img.copy()
            ...
            # Find best match with thread-local data copy
            query_embedding = faces[0].embedding
            
            # *** NEW: Check recent cache first for batch processing too ***
            recent_match = self.check_recent_cache(query_embedding)
            if recent_match:
                print(f"[BATCH CACHE HIT] {filename} matched recent cache with similarity {recent_match.get('recent_cache_similarity', 0):.3f}")
                
                identity = os.path.splitext(recent_match.get('matched_filename', ''))[0]
                ...
                ws_event_data = {
                    ...
                    "camera_id": None,
                    "camera_name": "Batch Process",
                    "confidence": recent_match.get('confidence', 0.0),
                    "matched_filename": recent_match.get('matched_filename', ''),
                    "identity": identity,
                    "input_image": f"/Get/In/{filename}",
                    "matched_image": f"Database/images/{recent_match.get('matched_filename', '')}",
                    "recent_cache_hit": True,
                    "request_id": request_id
                }
                ...
                return {
                    ...
                    "filename": filename,
                    "confidence": recent_match.get('confidence', 0.0),
                    "matchedFilename": recent_match.get('matched_filename', ''),
                    "recent_cache_hit": True,
                    "ws_event_data": ws_event_data,
                    "request_id": request_id
                }
            ...
            # Create a local copy of the frequent faces cache to avoid race conditions
            with threading.Lock():
                frequent_faces_cache_copy = copy.deepcopy(self.frequent_faces_cache)
            
            best_match = None
            best_similarity = -1
            best_identity = None
            
            # Use the copied cached normalized embeddings for faster comparison
            for identity, data in frequent_faces_cache_copy.items():
            ...
            if best_match and best_similarity > 0.4:
                ...
                # *** NEW: Add this successful batch match to recent cache ***
                batch_result = {
                    "status": "matched",
                    "confidence": float(best_similarity),
                    "matched_filename": os.path.basename(best_match['image_path'])
                }
                self.add_to_recent_cache(query_embedding, batch_result)
    ...
    def get_performance_metrics(self):
        ...
        return {
            ...
            "recent_recognition_cache": cache_stats
        }
...
def cache_stats():
    ...
    return jsonify(face_server.get_cache_statistics())

@app.route('/api/cache/clear', methods=['POST'])
def clear_recent_cache():
    """Clear the recent recognition cache"""
    try:
        cleared_count = len(face_server.recent_recognition_cache)
        face_server.recent_recognition_cache.clear()
        face_server.cache_hit_count = 0
        face_server.cache_miss_count = 0
        return jsonify({
            "status": "success",
            "message": f"Cleared {cleared_count} entries from recent cache"
        })
    except Exception as e:
...
if __name__ == '__main__':
    ...
    print(f"Recent recognition cache initialized with {face_server.recent_cache_duration}s duration")
    print(f"Cache similarity threshold: {face_server.recent_cache_similarity_threshold}")
